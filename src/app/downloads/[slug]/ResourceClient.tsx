...
import type { downloadResources } from "@/data/downloadResources";
...
export default function ResourceDetailPage({
  ...
  resource: (typeof downloadResources)[string];
}) {
  ...
  return (
    <div
      className="min-h-screen bg-background pb-12 animate-fade-in text-right"
      dir="rtl"
    >
                ...
                <thead>
                  <tr className="bg-secondary/60 text-foreground border-b border-border">
                    {resource.tableData.headers.map((header, idx) => (
                      <th
                        key={idx}
                        className="p-3 font-bold border-l border-border/40 last:border-l-0"
                      >
                        {header}
                      </th>
                    ))}
                  ...
                  {resource.tableData.rows.map((row, rowIdx) => (
                    ...
                    >
                      {row.map((cell, cellIdx) => (
                        <td
                          key={cellIdx}
                          className="p-3 border-l border-border/40 last:border-l-0 text-muted-foreground"
                        >
                          {cell}
                        </td>
                      ))}
